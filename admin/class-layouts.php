...
final class TWT_TCRM_Admin_Layouts {
  ...
  public static function enqueue_assets($hook) {
    // Só na nossa página
    if (!is_admin()) return;
    if (!isset($_GET['page']) || sanitize_text_field($_GET['page']) !== self::PAGE_SLUG) return;
  ...
  public static function handle_post() {
    ...
    if (!isset($_GET['page']) || sanitize_text_field($_GET['page']) !== self::PAGE_SLUG) {
      return;
    }
    ...
    // Reset
    if (isset($_GET['twt_tcrm_reset']) && $_GET['twt_tcrm_reset'] === '1') {
      $brand_id = isset($_GET['brand_id']) ? (int) $_GET['brand_id'] : 0;
      $campaign_id = isset($_GET['campaign_id']) ? (int) $_GET['campaign_id'] : 0;
    ...
    // Save
    if (!isset($_POST['twt_tcrm_do']) || sanitize_text_field($_POST['twt_tcrm_do']) !== 'save_layout') return;

    $nonce = isset($_POST['twt_tcrm_layout_nonce']) ? sanitize_text_field($_POST['twt_tcrm_layout_nonce']) : '';
  ...
  private static function number_field($label, $key, $layout, $min, $max, $step) {
    ...
    echo '<p>';
    echo '<label><strong>' . esc_html($label) . '</strong></label><br>';
    echo '<input type="number" name="' . esc_attr($key) . '" value="' . esc_attr($val) . '" min="' . esc_attr((int)$min) . '" max="' . esc_attr((int)$max) . '" step="' . esc_attr((int)$step) . '" style="width:100%;">';
    echo '</p>';
  }
