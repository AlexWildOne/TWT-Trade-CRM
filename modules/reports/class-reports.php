...
final class TWT_TCRM_Reports {
  ...
  private static function has_filter($arr, $key) {
  ...
  public static function get_submissions($args = [], $limit = 50, $offset = 0) {
    ...
    // campaign_id: permitir 0 (sem campanha)
    if (self::has_filter($args, 'campaign_id')) {
  ...
  public static function aggregate_by_question($filters = []) {
    ...
    // campaign_id: permitir 0 (sem campanha)
    if (self::has_filter($filters, 'campaign_id')) {
    ...
    // Para métricas numéricas (number/currency/percent) usamos um "any_value" só quando existe um destes.
    $sql = "
      SELECT
        ...
        SUM(
          CASE
            WHEN a.value_number IS NOT NULL THEN a.value_number
            WHEN a.value_currency IS NOT NULL THEN a.value_currency
            WHEN a.value_percent IS NOT NULL THEN a.value_percent
            ELSE 0
          END
        ) AS sum_any,
